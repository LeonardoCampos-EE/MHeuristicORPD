...
class Optimizer:
    def __init__(self, population_size, dim, objective_function, constraints = None, upper_bounds = None, lower_bounds = None):
        ...
        self.pop_array = (upper_bounds.T - lower_bounds.T)*\
                                self.random_gen.random(size = (dim, population_size), dtype = np.float64) +\
                                lower_bounds.T
        ...
        if constraints is not None:
            ...
            self.constraint_array = np.zeros( shape = (1, population_size) )
